...
from elearning.apps.actions.utils import create_action
...
class LoginView(RedirectAuthenticatedUserMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        signup_url = passthrough_next_redirect_url(self.request,
                                                   reverse("allauth:account_signup"),
...
class EmailView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    success_url = reverse_lazy('allauth:account_email')
...
class PasswordChangeView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    success_url = reverse_lazy("allauth:account_change_password")
    ...
    def render_to_response(self, context, **response_kwargs):
        if not self.request.user.has_usable_password():
            return HttpResponseRedirect(reverse('allauth:account_set_password'))
...
class PasswordSetView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    success_url = reverse_lazy("allauth:account_set_password")
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.request.user.has_usable_password():
            return HttpResponseRedirect(reverse('allauth:account_change_password'))
...
class PasswordResetFromKeyView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    success_url = reverse_lazy("allauth:account_reset_password_from_key_done")
    ...
    def get_context_data(self, **kwargs):
        ...
        ret['action_url'] = reverse(
            'allauth:account_reset_password_from_key',
...
class LogoutView(TemplateResponseMixin, View):
    ...
    def logout(self):
        ...
        create_action(self.request.user, 'has logged out')
